pipeline {
    ...
    stages {
        stage('Clone Repo') {
            steps {
                git branch: 'main', url: 'https://github.com/ankamraghu27/terraform_ci.git'
            }
        }
        ...
        stage('Configure VMs') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'ssh-ubuntu', keyFileVariable: 'UBUNTU_KEY')]) {
                    withCredentials([sshUserPrivateKey(credentialsId: 'ssh-ec2', keyFileVariable: 'AMAZON_KEY')]) {
                        dir('ansible') {
                            sh '''
